...
export default function CategoriesPage() {
  ...
  const [categories, setCategories] = useState<CategoryItem[]>(() => {
    ...
    return baseList.map((c) => ({
      ...c,
      iconUrl: c.iconUrl || "",
      secondImageIconUrl: c.secondImageIconUrl || "https://images.unsplash.com/photo-1581094288338-2314dddb7ece?w=120&auto=format&fit=crop&q=80",
    }));
  });
  ...
  const openAddDrawer = () => {
    ...
    setIcon("Wrench");
    setPrimaryIconUrl("");
    setSecondIconUrl("https://images.unsplash.com/photo-1581094288338-2314dddb7ece?w=120&auto=format&fit=crop&q=80");
    setStatus("Active");
  ...
  const openEditDrawer = (cat: CategoryItem) => {
    ...
    setSecondIconUrl(cat.secondImageIconUrl || "https://images.unsplash.com/photo-1581094288338-2314dddb7ece?w=120&auto=format&fit=crop&q=80");
  ...
  const handleIconUpload = (catId: string, file: File, type: "primary" | "second") => {
      ...
      setCategories((prev) =>
        prev.map((c) =>
          c.id === catId
            ? {
                ...c,
                ...(type === "primary" ? { iconUrl: result } : { secondImageIconUrl: result }),
              }
            : c
        )
      );
      ...
      const updatedList = categories.map((c) =>
        c.id === editingCategory.id
          ? {
              ...
              secondImageIconUrl: secondIconUrl,
              status,
  ...
  const catColumns: Column<CategoryItem>[] = [
    {
      key: "name",
      header: "Category Title & Dual-Tone Icons",
      accessor: (row) => (
        <div className="flex items-center gap-3">
          <div className="flex items-center -space-x-2 shrink-0">
            <div className="w-10 h-10 rounded-2xl overflow-hidden border-2 border-purple-500 bg-slate-50 dark:bg-slate-800 shadow-sm z-10 flex items-center justify-center p-1">
              {row.iconUrl ? (
            ...
            {row.secondImageIconUrl && (
              <div className="w-8 h-8 rounded-2xl overflow-hidden border-2 border-purple-400 bg-white dark:bg-slate-800 shadow-sm p-0.5">
                <img
            ...
            {categories.map((cat) => (
              ...
              >
                ...
                <div className="relative w-full h-36 rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center justify-center p-4 shadow-xs group-hover:scale-103 transition-transform">
                  ...
                  {/* Optional Second Image Icon Badge Overlay (Top Right) */}
                  {cat.secondImageIconUrl && (
                    <div className="absolute top-2 right-2 w-7 h-7 rounded-xl bg-purple-50 dark:bg-purple-950 border border-purple-200 dark:border-purple-800 p-0.5 shadow-sm">
                      <img
                        src={cat.secondImageIconUrl}
                        alt="2nd Badge"
                        className="w-full h-full object-contain"
                      />
                    </div>
                ...
                <div className="pt-3 pb-2 flex items-center gap-2">
                  <label className="flex-1 py-2 px-3 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-extrabold text-[11px] flex items-center justify-center gap-1.5 cursor-pointer shadow-xs transition-colors">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (file) handleIconUpload(cat.id, file, "primary");
                      }}
                    />
                  </label>

                  <label className="py-2 px-2.5 rounded-xl bg-purple-50 dark:bg-purple-950/60 hover:bg-purple-100 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800 font-bold text-[10px] flex items-center justify-center gap-1 cursor-pointer shrink-0" title="Upload 2nd Image Icon Badge">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const file = e.target.files?.[0];
                        if (file) handleIconUpload(cat.id, file, "second");
                      }}
                    />
                  </label>
                </div>
                ...
                <div className="flex items-center justify-between pt-1 border-t border-slate-200/60 dark:border-slate-700/60">
                  ...
                  <button
                    ...
                    className="w-8 h-8 rounded-full border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400 group-hover:text-purple-600 group-hover:border-purple-300 dark:group-hover:text-purple-400 transition-colors cursor-pointer"
            ...
            >
              ...
              <div className="flex-1 p-6 overflow-y-auto space-y-6">
                ...
                <div className="p-5 rounded-2xl bg-brand-50/50 dark:bg-brand-950/30 border border-brand-200 dark:border-brand-800 space-y-4">
                  ...
                  {/* Second Image Icon Upload */}
                  <div className="space-y-2 pt-2 border-t border-brand-200/60 dark:border-brand-900/60">
                    <label className="text-[11px] font-bold text-slate-700 dark:text-slate-300 flex items-center justify-between">
                    ...
                    <div className="flex gap-2 items-center">
                      <div className="w-12 h-12 rounded-xl overflow-hidden border-2 border-purple-500 shrink-0 bg-white dark:bg-slate-800 p-0.5">
                        <img
                          src={secondIconUrl}
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (file) {
                              const reader = new FileReader();
                              reader.onload = (re) => {
                                setSecondIconUrl(re.target?.result as string);
                              };
                              reader.readAsDataURL(file);
                            }
                          }}
                      ...
                      <input
                        type="url"
                        value={secondIconUrl}
                        onChange={(e) => setSecondIconUrl(e.target.value)}
