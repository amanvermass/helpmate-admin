...
export default function FaqPage() {
  ...
  return (
    <div className="space-y-6">
      {/* PAGE HEADER */}
      <div className="p-5 sm:p-6 rounded-3xl bg-gradient-to-r from-emerald-600 via-teal-700 to-cyan-700 text-white shadow-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative overflow-hidden">
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-slate-400">— controls the heading shown above FAQs on the website</span>
          </div>
          {!editingHeader ? (
            <button type="button" onClick={() => { setHeaderDraft(header); setEditingHeader(true); }} className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:text-brand-600 cursor-pointer transition-colors">
              <Edit2 className="w-3 h-3" /> Edit
            </button>
          ) : (
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => setEditingHeader(false)} className="px-3 py-1.5 rounded-xl text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 cursor-pointer transition-colors">Cancel</button>
              <button type="button" onClick={saveHeader} className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold bg-teal-600 hover:bg-teal-700 text-white cursor-pointer transition-colors"><Save className="w-3 h-3" />Save</button>
            </div>
          )}
        </div>
        ...
        {!editingHeader ? (
          <div className="px-5 py-4 flex flex-wrap gap-6">
            {[["Badge", header.badge], ["Title", header.title], ["Subtitle", header.subtitle], ["View All Text", header.viewAllText]].map(([label, val]) => (
              <div key={String(label)}>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider mb-0.5">{label}</p>
                <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 max-w-xs">{val}</p>
              </div>
            ))}
          </div>
        ) : (
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
          {["All", ...CATEGORIES].map((cat) => (
            <button key={cat} type="button" onClick={() => setSelectedCategory(cat)}
              className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap cursor-pointer transition-colors shrink-0 ${selectedCategory === cat ? "bg-teal-600 text-white shadow-sm" : "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50"}`}>
              {cat}
              {cat === "All" ? <span className="ml-1 text-[10px] opacity-70">({faqs.length})</span>
                : faqs.filter(f => f.category === cat).length > 0 ? <span className="ml-1 text-[10px] opacity-70">({faqs.filter(f => f.category === cat).length})</span> : null}
            </button>
          ))}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <div className="relative">
            ...
            <input type="text" placeholder="Search FAQs..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 pr-3 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:outline-none focus:border-teal-500 w-48" />
          </div>
          <button type="button" onClick={() => setFaqModal({ open: true, mode: "add", data: { question: "", answer: "", category: "General", isActive: true, sortOrder: faqs.length + 1 } })}
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs cursor-pointer shadow-sm transition-colors shrink-0">
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2">
          ...
          <span className="text-[10px] text-slate-400 ml-1">— shows only active FAQs as they appear on site</span>
        </div>
        <div className="p-6 sm:p-8 space-y-4">
          <div className="text-center space-y-1 mb-6">
            <p className="text-[10px] font-black uppercase tracking-widest text-teal-600">{header.badge}</p>
            <h2 className="text-xl font-black text-slate-900 dark:text-white">{header.title}</h2>
            <p className="text-xs text-slate-500 max-w-md mx-auto">{header.subtitle}</p>
          </div>
          <div className="space-y-0 divide-y divide-slate-100 dark:divide-slate-800 border-t border-slate-100 dark:border-slate-800">
            {faqs.filter(f => f.isActive).sort((a, b) => a.sortOrder - b.sortOrder).slice(0, 5).map((faq) => (
              <div key={faq.id} className="py-3">
                <button type="button" onClick={() => setExpandedId(expandedId === faq.id ? null : faq.id)}
                  className="w-full flex items-center justify-between text-left cursor-pointer group">
                  <span className="font-bold text-sm text-slate-900 dark:text-white group-hover:text-teal-600 transition-colors pr-4">{faq.question}</span>
                  {expandedId === faq.id ? <ChevronUp className="w-4 h-4 text-teal-600 shrink-0" /> : <ChevronDown className="w-4 h-4 text-slate-400 shrink-0" />}
                </button>
                {expandedId === faq.id && (
                  <p className="text-xs text-slate-500 mt-2 leading-relaxed pr-6">{faq.answer}</p>
                )}
              </div>
            ))}
          </div>
          <div className="text-center pt-2">
            <span className="text-xs font-bold text-teal-600 cursor-pointer hover:underline">{header.viewAllText}</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between px-1">
          <p className="text-xs font-bold text-slate-500">{filtered.length} FAQ{filtered.length !== 1 ? "s" : ""} found</p>
        </div>
        ...
        {filtered.length === 0 && (
          <div className="rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800 p-12 text-center text-slate-400 space-y-2">
            <MessageCircleQuestion className="w-8 h-8 mx-auto opacity-40" />
          ...
          <div key={faq.id} className={`rounded-2xl border bg-white dark:bg-slate-900 shadow-xs transition-all ${faq.isActive ? "border-slate-200 dark:border-slate-800" : "border-dashed border-slate-300 dark:border-slate-700 opacity-60"}`}>
            <div className="flex items-start gap-3 p-4">
              ...
              <div className="flex flex-col gap-0.5 shrink-0 pt-0.5">
                <button type="button" onClick={() => moveUp(faq.id)} disabled={idx === 0} className="p-0.5 rounded text-slate-400 hover:text-teal-600 disabled:opacity-30 cursor-pointer disabled:cursor-not-allowed transition-colors"><ArrowUp className="w-3 h-3" /></button>
                <span className="text-[10px] font-black text-slate-400 text-center w-full">{faq.sortOrder}</span>
                <button type="button" onClick={() => moveDown(faq.id)} disabled={idx === filtered.length - 1} className="p-0.5 rounded text-slate-400 hover:text-teal-600 disabled:opacity-30 cursor-pointer disabled:cursor-not-allowed transition-colors"><ArrowDown className="w-3 h-3" /></button>
              </div>
              ...
              <div className="flex-1 min-w-0 space-y-1.5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${faq.isActive ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400" : "bg-slate-100 dark:bg-slate-800 text-slate-400"}`}>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button type="button" onClick={() => setFaqModal({ open: true, mode: "edit", data: { ...faq } })} className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-brand-600 transition-colors cursor-pointer" title="Edit"><Edit2 className="w-3.5 h-3.5" /></button>
                <button type="button" onClick={() => toggleActive(faq.id)} className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-teal-600 transition-colors cursor-pointer" title={faq.isActive ? "Hide" : "Show"}>
                  {faq.isActive ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}
                </button>
                <button type="button" onClick={() => setDeleteModal({ open: true, id: faq.id, question: faq.question })} className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-red-600 transition-colors cursor-pointer" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            </div>
          </div>
        ))}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800">
                ...
                <button type="button" onClick={() => setFaqModal({ open: false, mode: "add", data: {} })} className="p-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer transition-colors"><X className="w-4 h-4 text-slate-500" /></button>
              </div>
              ...
              <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
                <div>
                  ...
                  <input type="text" placeholder="What is HelpMate's..." value={faqModal.data.question || ""} onChange={(e) => setFaqModal({ ...faqModal, data: { ...faqModal.data, question: e.target.value } })} className={inputCls} />
                </div>
                ...
                <div>
                  ...
                  <textarea rows={5} placeholder="Provide a clear, detailed answer..." value={faqModal.data.answer || ""} onChange={(e) => setFaqModal({ ...faqModal, data: { ...faqModal.data, answer: e.target.value } })} className={`${inputCls} resize-none`} />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-bold text-slate-700 dark:text-slate-300 block mb-1">Category</label>
                    <select value={faqModal.data.category || "General"} onChange={(e) => setFaqModal({ ...faqModal, data: { ...faqModal.data, category: e.target.value } })} className={inputCls}>
                      {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-xs font-bold text-slate-700 dark:text-slate-300 block mb-1">Sort Order</label>
                    <input type="number" placeholder="1" value={faqModal.data.sortOrder || ""} onChange={(e) => setFaqModal({ ...faqModal, data: { ...faqModal.data, sortOrder: Number(e.target.value) } })} className={inputCls} />
                  </div>
                </div>

                <div className="flex items-center gap-2">
                  <input type="checkbox" id="faq-active" checked={faqModal.data.isActive ?? true} onChange={(e) => setFaqModal({ ...faqModal, data: { ...faqModal.data, isActive: e.target.checked } })} className="w-4 h-4 rounded accent-teal-600 cursor-pointer" />
              ...
              <div className="px-5 py-4 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-2">
                <button type="button" onClick={() => setFaqModal({ open: false, mode: "add", data: {} })} className="px-4 py-2 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 cursor-pointer transition-colors">Cancel</button>
                <button type="button" onClick={saveFaq} className="px-4 py-2 rounded-xl text-xs font-bold text-white bg-teal-600 hover:bg-teal-700 flex items-center gap-1.5 cursor-pointer transition-colors"><Save className="w-3.5 h-3.5" />Save FAQ</button>
              </div>
            </div>
          </div>
        </Portal>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-sm p-6 space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={deleteFaq} className="flex-1 py-2.5 rounded-xl text-xs font-bold text-white bg-red-600 hover:bg-red-700 cursor-pointer transition-colors">Delete</button>
              </div>
            </div>
          </div>
        </Portal>
