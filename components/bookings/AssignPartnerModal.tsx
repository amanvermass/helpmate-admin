...
interface AssignPartnerModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onPartnerAssigned: (bookingId: string, technician: Technician | null) => void;
}
...
export function AssignPartnerModal({
  ...
  // Find current technician ID if booking already has an assigned technician
  const currentTech = initialTechnicians.find(
    (t) => t.id === booking?.technicianId || t.name === booking?.technicianName
  );

  const [selectedTechId, setSelectedTechId] = useState<string>(currentTech?.id || "");

  // Reset selectedTechId when modal opens with a new booking
  React.useEffect(() => {
    if (booking) {
      const match = initialTechnicians.find(
        (t) => t.id === booking.technicianId || t.name === booking.technicianName
      );
      setSelectedTechId(match?.id || "");
      setSearchQuery("");
    }
  }, [booking]);
        ...
        <div className="bg-white dark:bg-slate-900 ring-1 ring-slate-900/10 dark:ring-slate-800 rounded-3xl max-w-lg w-full p-6 space-y-5 shadow-2xl animate-in zoom-in-95 duration-200 outline-none">
          ...
          <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs space-y-1">
            <div className="flex items-center justify-between">
              ...
              {booking.technicianName && (
                <span className="text-[10px] font-extrabold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-2 py-0.5 rounded border border-emerald-200 dark:border-emerald-800 flex items-center gap-1">
                  <ShieldCheck className="w-3 h-3" />
                  Currently: {booking.technicianName}
                </span>
              )}
            </div>
          ...
          <div className="flex items-center gap-2 pt-2">
            ...
            <button
              ...
              className="flex-1 py-2.5 rounded-xl bg-brand-500 hover:bg-brand-600 disabled:opacity-50 text-white text-xs font-bold shadow-lux cursor-pointer transition-all"
            >
              {isReassign ? "Update Partner Assignment" : "Confirm Partner Assignment"}
            </button>
          </div>
        </div>
      </div>
